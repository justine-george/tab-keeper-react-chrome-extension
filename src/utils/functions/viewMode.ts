...
// Importing is safe; CALLING isTabView() is not -- it reads
// `window.location`, which does not exist in the worker, so the worker must
// never invoke it.
...
/**
 * The tab most recently used in this window, among the tabs `exclude` does
 * NOT match (D15) -- the source for a "New tab" name suggestion when the tab
 ...
 *
 * `exclude` is a PREDICATE, not an id: the tab view's own tab is always a
 * Tab Keeper page by definition, so excluding by ADDRESS (isTabKeeperPage,
 * capture.ts) already covers it without needing to know its id -- and also
 * catches a SECOND Tab Keeper page open in the same window, which an id
 * comparison never could. It is also what lets the POPUP reuse this same
 * function: the popup has no "own id" to compare against, only an active
 * tab that may or may not itself be a Tab Keeper page.
 */
